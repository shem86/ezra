...
> **ARCHIVED — historical record, not current guidance.** Executed 2026-07-13/14;
> the repo is public as [`shem86/ezra`](https://github.com/shem86/ezra). Restored
> to the tree 2026-07-21 (it was deleted in `280a265`, the pre-public doc trim, so
> the audit record survived only in history). Kept because it is the *only* record
> of what was audited and what was **consciously accepted** — notably the decision
> to publish the AWS/host identifiers (§6) and to defer SSH hardening (Phase E).
> Status lines below are as-written at execution time; current state lives in
> [`STATUS.md`](../../../STATUS.md).
>
> Two acceptance criteria in §5 were **never met** and remain open — the tool-based
> secret re-scan and the broad PII sweep. See `STATUS.md`.
