...
> **ARCHIVED — historical record, not current guidance.** The console shipped.
> Restored to the tree 2026-07-21: this file was deleted in `280a265` (the
> pre-public doc trim) while `README.md` still linked it, leaving a broken link
> on a public repo. Current state lives in [`STATUS.md`](../../../STATUS.md).
> Its companions `backoffice-plan.md` and `backoffice-tasks.md` remain deleted
> (process docs, nothing links them); they survive in history at `280a265^`.

Status: **APPROVED** (2026-06-23) — Phase 1 complete, all decisions confirmed.
Plan: `docs/backoffice-plan.md` (Phase 2 — deleted, see `280a265^`).
