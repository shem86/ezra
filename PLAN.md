...
Milestones M0–M6, each ending at a verification gate; no milestone starts on an unverified foundation. Number acquisition, warming, and soak testing are **out of scope** for this project — the transport is assumed paired and usable wherever a milestone needs it.

```
M0 scaffold → M1 spikes → M2 transport adapter + ops → M3 durable core
                            → M4 reasoning → M5 HITL → M6 wire-up → launch
(M2.5 host/backup and M5.5 calendar OAuth run parallel to M3–M5)
```
...
### M2 — Transport adapter + ops monitoring (standalone, agent-free)
Baileys socket behind the `src/transport/` interface, session-state persistence (writable volume, never backed up for restore), socket-health monitor, **independent alert channel** (Telegram/Pushover — pick at build time), dead-man ping to healthchecks.io-style service, forced-reconnect command. Runs standalone, no LLM, no DB writes.
**Gate:** alert fires on manual socket kill; dead-man fires on process kill; forced reconnect recovers cleanly.
...
### M6 — Wire-up and launch (requires M2, M2.5, and M5 gates)
Swap stub transport for the M2 Baileys adapter; durable-enqueue-before-ack against the real socket; send classes live (at-least-once: reminders, nags, approval prompts; at-most-once: echoes); recovery runbook written (`docs/recovery-runbook.md`) using sent-log + deterministic IDs; full restore drill incl. runbook reconciliation; deploy hardened process to host.
**Gate:** SPEC success-criteria checklist swept top to bottom. Launch is the checklist completing, not a date.
...
- M2, M2.5, and M5.5 float; all must land before M6.
...
Subagents, self-improving loops, code-execution tools (architecture decision 11), LLM-as-judge quality evals, any second conversation/group (the queue is keyed by conversation, so the seam exists; nothing else is built for it). Number acquisition, warming, and soak testing were declared out of scope on 2026-06-09 and removed from all project docs.
...
9. M6: SPEC success-criteria checklist fully swept.
